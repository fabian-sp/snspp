...
params_svrg = {'n_epochs' : 80, 'batch_size': 50, 'alpha': 0.022}

params_adagrad = {'n_epochs' : 170, 'batch_size': 240, 'alpha': 0.028}

params_snspp = {'max_iter' : 100, 'batch_size': 240, 'sample_style': 'constant', 'alpha' : 7.,\
                "reduce_variance": True}

#params_tuner(f, phi, A, solver = "adagrad", batch_range = np.array([50, 250, 500]))
#params_tuner(f, phi, A, solver = "snspp", alpha_range = np.linspace(4, 9, 10), batch_range = np.array([240, 480]))
...
if _plot:
    ...
    if _save:
        fig.savefig('../data/plots/exp_gisette/obj2.pdf', dpi = 300)
    ...
    if _save:
        fig.savefig('../data/plots/exp_gisette/error.pdf', dpi = 300)
    ...
    if _save:
        fig.savefig('../data/plots/exp_gisette/accuracy.pdf', dpi = 300)
    ...
    if _save:
        fig.savefig('../data/plots/exp_gisette/coeff.pdf', dpi = 300)

...
#########################################################################
    ...
    def plot_fnat(self, ax = None, runtime = True, median = False, markersize = 3, markevery_dict = dict(), ls = '-', lw = 0.4, log_scale = False, sigma = 0):
        ...
        for s in self.solvers:
            ...
            all_fnat = np.vstack([this_res[k]["fnat"] for k in range(K)])
            
            if median:
                y = np.median(all_fnat, axis=0)
            else:
                y = all_fnat.mean(axis=0)
                
            all_std = all_fnat.std(axis=0)
        ...
        ax.set_ylabel(r"$\|F_{\mathrm{nat}}(x^k)\|$", fontsize = 12)
